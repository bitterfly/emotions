...
func getData(bucketSize int, frameLen int, frameStep int, files []string, tag string) []float64 {
	numElectrodes := 19
	numWaves := 4
...
// ei = 19x4
func normaliZeDataByWaves(e1 []float64, e2 []float64, e3 []float64) {
...
func main() {
	if len(os.Args) < 3 {
		panic("go run main.go <ouput-dir> <input-file>\n<input-file>:<emotion>	<csv-file>")
	}

	outputDir := os.Args[1]
	arguments, _, err := emotions.ParseArgumentsFromFile(os.Args[2], false)
	...
	positiveData := getData(1, frameLen, frameStep, eegPositive, "eeg-positive")
	negativeData := getData(1, frameLen, frameStep, eegNegative, "eeg-negative")
	neutralData := getData(1, frameLen, frameStep, eegNeutral, "eeg-neutral")

	fmt.Printf("%d\n", len(positiveData))

	normaliZeDataByWaves(positiveData, negativeData, neutralData)
	drawEmotion("eeg-positive", positiveData, outputDir)
