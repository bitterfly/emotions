...
\begin{flalign*}
	...
	& = \cfrac{\partial H_p(Y|X)}{\partial p(y_0 | x_0)} + \cfrac{\partial\B{\lambda_i (E(p, h_i) - E(\tilde{p}, h_i ))}}{\partial p(y_0 | x_0)} + \cfrac{\B{\sum\limits_{x\in X} \mu_x \sum\limits_{y \in Y} p(y|x) -1}}{\partial p(y_0 | x_0)}&&\\
	\\
...
\begin{flalign}
	...
	\nonumber & log(p(y_0 | x_0)) = \sum\limits_{i=1}^K \lambda_i h_i(x_0, y_0) - 1 + \cfrac{\mu_{x_0}}{\tilde{p}(x_0)} \longleftrightarrow&&\\
	& p(y_0 | x_0) = \exp\B{\sum\limits_{i=1}^K \lambda_i h_i(x_0, y_0)}\exp\B{- 1 + \cfrac{\mu_{x_0}}{\tilde{p}(x_0)}}
\end{flalign}

Производната по $\mu_{x_0}$ ни дава:
\begin{flalign*}
	&\sum\limits_{y\in Y} p(y | x_0) = 1 \longleftrightarrow && \\
	& \sum\limits_{y\in Y} \exp\B{\sum\limits_{i=1}^K \lambda_i h_i(x_0, y) - 1 + \cfrac{\mu_{x_0}}{\tilde{p}(x_0)}} = 1 \longleftrightarrow && \\
	& exp\B{-1 + \cfrac{\mu_{x_0}}{\tilde{p}(x_0)}}\sum\limits_{y\in Y} \exp\B{\sum\limits_{i=1}^K \lambda_i h_i(x_0, y)} = 1 &&\\
	& \longleftrightarrow &&\\
	& exp\B{-1 + \cfrac{\mu_{x_0}}{\tilde{p}(x_0)}} = \cfrac{1}{\sum\limits_{y\in Y} \exp\B{\sum\limits_{i=1}^K \lambda_i h_i(x_0, y)}} &&\\
