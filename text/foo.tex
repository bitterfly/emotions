...
\label{chap:speech}
   ...
   Това, от което се интересуваме, е $\mathcal{V}$.
    ...
    \begin{subequations}
        \label{eq:2:2:01}
        \begin{align}
            \label{eq:2:2:01:a} \cfrac{\partial\rho}{\partial x} & = \cfrac{\rho}{A} \cfrac{\partial u}{\partial t}\\
            \label{eq:2:2:01:b} \cfrac{\partial u}{\partial x} & = \cfrac{A}{\rho \mathcal{c}^2} \cfrac{\partial \rho}{\partial t}
        \end{align}
    \end{subequations}
    ...
    \begin{align}
        ...
        \label{eq:2:2:05}
        & \cfrac{A_{k+1}}{A_{k}}[u_{k}^{+}(t - \tau_k) + u_{k}^{-}(t + \tau_k)] = u_{k+1}^{+}(t) + u_{k+1}^{-}(t)
    \end{align}
    ...
    \begin{flalign}
        ...
        & \nonumber u_{k+1}^{+}(t) = \cfrac{A_{k+1}}{A_k} u_{k}^{+}(t - \tau_k) + \cfrac{A_{k+1}}{A_k}[u_{k}^{+}(t - \tau_k) - u_{k+1}^{+}(t)  + u_{k+1}^{-}(t)] - u_{k+1}^{-}(t)  && \\
        & \nonumber u_{k+1}^{+}(t)\Q{1 + \cfrac{A_{k+1}}{A_k}} = u_{k}^{+}(t - \tau_k)\cfrac{2A_{k+1}}{A_k} + u_{k+1}^{-}(t)\Q{\cfrac{A_{k+1}}{A_k} - 1}  && \\
        & \nonumber u_{k+1}^{+}(t)\Q{\cfrac{A_k + A_{k+1}}{A_k}} = u_{k}^{+}(t - \tau_k)\cfrac{2A_{k+1}}{A_k} + u_{k+1}^{-}(t)\Q{\cfrac{A_{k+1} - A_k}{A_k}} && \\
        & \label{eq:2:2:07} u_{k+1}^{+}(t) = u_{k}^{+}(t - \tau_k)\Q{\cfrac{2A_{k+1}}{A_k + A_{k+1}}} + u_{k+1}^{-}(t)\Q{\cfrac{A_{k+1} - A_k}{A_k + A_{k+1}}} &&
    \end{flalign}
    ...
    \begin{subequations}
        ...
        \begin{flalign}
            ...
            &  u_k^{+}(t - \tau_k) = u_{k+1}^{+}(t)\Q{\cfrac{A_k + A_{k+1}}{2A_{k+1}}} + u_{k+1}^{-}(t)\Q{\cfrac{A_k - A_{k+1}}{2A_{k+1}}} \label{eq:2:2:10:a} && \\
            & \nonumber \intertext{Заместваме $\eqref{eq:2:2:10:a}$ в $\eqref{eq:2:2:04}$}
            & \nonumber u_k^{-}(t + \tau_k) = u_k^{+}(t - \tau_k) - u_{k+1}^{+}(t) + u_{k+1}^{-}(t) && \\
            & \nonumber u_k^{-}(t + \tau_k) = u_{k+1}^{+}(t)\Q{\cfrac{A_k + A_{k+1}}{2A_{k+1}}} + u_{k+1}^{-}(t)\Q{\cfrac{A_k - A_{k+1}}{2A_{k+1}}} - u_{k+1}^{+}(t) + u_{k+1}^{-}(t) && \\
            & \nonumber u_k^{-}(t + \tau_k) = u_{k+1}^{+}(t)\Q{\cfrac{A_k + A_{k+1} - 2A_{k+1}}{2A_{k+1}}} + u_{k+1}^{-}(t)\Q{\cfrac{A_k - A_{k+1} + 2A_{k+1}}{2A_{k+1}}} && \\
            & \label{eq:2:2:10:b} u_k^{-}(t + \tau_k) = u_{k+1}^{+}(t)\Q{\cfrac{A_k - A_{k+1}}{2A_{k+1}}} + u_{k+1}^{-}(t)\Q{\cfrac{A_k + A_{k+1}}{2A_{k+1}}} &&
        \end{flalign}
    \end{subequations}
    ...
    \begin{equation}
        \label{eq:2:2:14}
        r_L = \B{\cfrac{\rho\mathcal{c} - A_L Z_L}{\rho\mathcal{c} + A_L Z_L}}
    \end{equation}
    ...
    \begin{align}
    \label{eq:2:2:16}
        U(0, \mathcal{z}) =  U_G(\mathcal{z}) - \cfrac{P(0, \mathcal{z})}{Z_G(\mathcal{z})},
    \end{align}
    ...
    \begin{flalign}
        \label{eq:2:2:17}
        & \nonumber u_1^{+}(t) - u_1^{-}(t) = u_G(t) - \cfrac{\rho \mathcal{c}}{A_1}\Q{\cfrac{u_1^{+}(t) + u_1^{-}(t)}{Z_G}} && \\
        & \nonumber u_1^{+}(t)\Q{1 + \cfrac{\rho\mathcal{c}}{A_1 Z_G}}  = u_G(t) + u_1^{-}(t)\Q{1 - \cfrac{\rho\mathcal{c}}{A_1 Z_G}} && \\
        & \nonumber u_1^{+}(t) = u_G(t)\Q{\cfrac{A_1 Z_G}{A_1 Z_G + \rho \mathcal{c}}} + u_1^{-}(t)\Q{\cfrac{A_1 Z_G - \rho \mathcal{c}}{A_1 Z_G + \rho \mathcal{c}}} &&\\
        & \nonumber u_1^{+}(t) = u_G(t) \Q{\cfrac{1 + r_G}{2}} + r_G u_1^{-}(t)&& \\
        & U_G(\mathcal{z}) = \Q{\cfrac{2}{1 + r_G}}U_1^{+}(\mathcal{z}) - \Q{\cfrac{2r_G}{1 + r_G}}U_1^{-}(\mathcal{z}), \text{където} &&
    \end{flalign}
    \begin{equation*}
    ...
    За да видим общия вид на $\mathcal{V}$, нека всички тръби имат равна дължина и $\tau_i = \cfrac{1}{2}, i \in[1...N]$
    ...
    \begin{flalign}
        \label{eq:2:2:20}
        & \cfrac{1}{\mathcal{V}(\mathcal{z})} = \cfrac{U_G(\mathcal{z})}{U_L(\mathcal{z})}  = \left[
            \begin{array}{cc}
                \cfrac{2}{1+r_G}, & -\cfrac{2r_G}{1 + r_G} \\
            \end{array}
    ...
    \begin{flalign}
        \label{eq:2:2:21}
        & \cfrac{1}{\mathcal{V}(\mathcal{z})} = \cfrac{U_G(\mathcal{z})}{U_L(\mathcal{z})}  = \mathcal{z}^{N/2}\left[
            \begin{array}{cc}
                \cfrac{2}{1+r_G}, & -\cfrac{2r_G}{1 + r_G} \\
            \end{array}
    ...
    \begin{align}
        \label{eq:2:2:23}
        \mathcal{V}(\mathcal{z}) = \cfrac{0.5(1+r_G)\prod\limits_{i=1}^{N}{(1 + r_i)} \mathcal{z}^{-N/2}}{1 - \sum\limits_{i=1}^{N}{\alpha_i \mathcal{z}^{-i}}}
    \end{align}
    ...
    \section{Общ вид на $\mathcal{Y}$}
    От уравнения $\ref{eq:2:2:22}, \ref{eq:2:2:24}, \ref{eq:2:2:25}$ следва, че видът на  $\mathcal{Y}$ е
    ...
    зъбите. 
    Нека предположим, че вокалния тракт е линейна, времево-независима система и да разгледаме свойствата.
    ...
    в случая на вокалния тракт $\autoref{eq:2:4:1}$ има вида:
