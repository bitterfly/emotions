...
\begin{appendices}
    ...
    Нека $\mathcal{z} \in \mathbb{C}$. Видяхме,че предавателната функция $\mathcal{H}$ има вида
    \begin{flalign}
        \tag{\ref{eq:systems:6}}
        & \mathcal{H}(\mathcal{z}) =  \cfrac{\sum\limits_{m=0}^{M} b_m \mathcal{z}^{-m}}{\sum\limits_{k=0}^{N} a_k \mathcal{z}^{-k}} = && \\
        \nonumber & \mathcal{H}(\mathcal{z}) = \cfrac{N(\mathcal{z})}{D(\mathcal{z})} = B \cfrac{(\mathcal{z} - \beta_1)(\mathcal{z} - \beta_2)...(\mathcal{z} - \beta_M)}{(\mathcal{z} - \alpha_1)(\mathcal{z} - \alpha_2)...(\mathcal{z} - \alpha_N)}, &&
    \end{flalign}
