...
\begin{appendices}
    ...
    \begin{figure}[H]%
        \centering
            ...
            \caption{Действие на IIR филтър от втори ред за $\alpha_1 \in \{(0.25 + 0.5i), (0.5 + 0.75i), (0.375 + 0.625)\}$}
            \label{fig:appendix:2:3}
    \end{figure}  
    ...
    \begin{figure}[H]%
        ...
        \subfloat{%
            \begin{tikzpicture}[baseline]
                \begin{axis}[
                    xlabel={Графика на $g$ с отдалечаващи се от реалната ос полюси},
                    ylabel={Амплитуда},
                    xmin=0, xmax=4,
                    ymin=0, ymax=5,
                    xtick={0, 2.5, 5},
        ...
        \caption{Действие на IIR филтър от втори ред за различни стойности на $a$ и $b$}
        \label{fig:appendix:2:3}
    \end{figure} 

    Местенето на полюсите по-далеч от реалната ос, раздалечава върховете по честотната лента.
    Видът на резонатора (тоест честотна ента, честота и амплитуда), се определят главно от полюсите.
    Нулите могат да влияят на графиката между двата върха на полюсите.
    ...
    по-лесно да се моделира чрез промяна на коефициентите. След това съчетаването им е просто произведение.
    % \begin{figure}[H]
    %     \centering
    %     \subfloat[Графика на $g(\omega)$ в $-\pi, \pi$]{%
    %     \begin{tikzpicture}[baseline]
    ...
    %             grid style=dashed,]
    %             \addplot [domain=-pi:pi, samples=500, thick]{( (1 - (1.17)*cos(deg(x)) - (-0.64)*cos(deg(2*x)))^2 + ((1.17)*sin(deg(x)) + (-0.64)*sin(deg(2*x)) )^2  )^(-1/2)};
    %             \addplot [domain=-pi:pi, samples=500, thick]{(i)* ( (1 - (1.17)*cos(deg(x)) - (-0.64)*cos(deg(2*x)))^2 + ((1.17)*sin(deg(x)) + (-0.64)*sin(deg(2*x)) )^2  )^(-1/2)};
    %             \end{axis}
    %     \end{tikzpicture}
    %     }
    % \end{figure}
