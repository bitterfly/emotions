...
if [ $# -le 7 ]; then
    echo "usage: <train-executable> <test-executable> <batch-dir> <speech-gmm-dir> <eeg-gmm-dir> <result-model-dir> <result-dir> [<k> <type> <feature-type>]"
    exit 1
fi
...
for file in $(find ${batch_files_dir} -type f | sort); do
    echo Testing ${file} 
    "${train_executable}" "${speech_gmm_dir}" ${eeg_gmm_dir}  ${gmm_models_dir}/gmm_$(basename ${file%.txt}) <(cat $(comm -23 <(echo ${batch_files} | sort) <(echo ${file})))
